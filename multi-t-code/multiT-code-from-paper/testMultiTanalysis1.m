function testMultiTanalysis1(subject,condition,reg) 
...
%% load mask
maskfn = fullfile(dataDir,'multiGroupRes','commonM_allSubs_MNI.nii.gz');
niifile = load_untouch_nii(maskfn);
niidata =  niifile.img;
[lidx, locations ] = getLocationsFromMaskNii(niidata);
resultsDirName=fullfile('../../multit/results');
...
if strcmp(condition,'hand') || strcmp(condition,'percept') || strcmp(condition,'ans')
        data_loc = fullfile(dataDir,num2str(subject), '/derive/PEs/multi_MNI_PEs', ['all_' pesType]); %for MNI space PEs
else % within hand conditions
        data_loc = fullfile(dataDir,num2str(subject), '/derive/PEs/multi_MNI_PEs/',[condition '_' pesType]);%for MNI space PEs
end
...
mulTout_dir=fullfile(dataDir,num2str(subject) ,'/derive/new_multiTres');
...
% if strcmp(reg,'nat')
%     TmapName=[condition '_tmap'];
% else
    TmapName=[condition '_MNI_tmap' num2str(params.numShuffels) 'shuffels'];
% end
